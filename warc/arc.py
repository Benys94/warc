...
ARC1_HEADER_RE = re.compile(
    r'(?P<url>\S*)\s(?P<ip_address>\S*)\s(?P<date>\S*)'
    r'\s(?P<content_type>\S*)\s(?P<length>\S*)'
)
ARC2_HEADER_RE = re.compile(
    r'(?P<url>\S*)\s(?P<ip_address>\S*)\s(?P<date>\S*)'
    r'\s(?P<content_type>\S*)\s(?P<result_code>\S*)\s(?P<checksum>\S*)'
    r'\s(?P<location>\S*)\s(?P<offset>\S*)\s(?P<filename>\S*)\s(?P<length>\S*)'
)
...
class ARCHeader(CaseInsensitiveDict):
    ...
    V2 header fields are 
        ...
        * checksum 
        * location
        * offset (offset from beginning of file to record)
        * filename (name of arc file)
        * length (length of the n/w doc in bytes)

    """
    def __init__(
        self, url="", ip_address="", date="", content_type="", result_code="",
        checksum="", location="", offset="", filename="", length="", version=2
    ):
        ...
        if isinstance(date, datetime.datetime):
            ...
            try:
                ...
                raise ValueError(
                    "Couldn't parse the date '%s' in file header" % date
                )

        self.version = version

        CaseInsensitiveDict.__init__(
            self, url=url, ip_address=ip_address, date=date,
    ...
    def write_to(self, f, version=None):
        ...
        if version == 1:
            header = (
                "%(url)s %(ip_address)s %(date)s %(content_type)s %(length)s"
            )
        elif version == 2:
            header = (
                "%(url)s %(ip_address)s %(date)s %(content_type)s "
    ...
    def __repr__(self):
        keys = [
            'url', 'ip_address', 'date', 'content_typeresult_code',
...
class ARCRecord(object):
    def __init__(self, header=None, payload=None, headers=None, version=None):
        headers = {} if headers is None else headers
        if not (header or headers):
            raise TypeError(
                "Can't write create an ARC1 record without a header"
            )
...
class ARCFile(object):
    def __init__(
        self, filename=None, mode=None, fileobj=None,
        version=None, file_headers=None
    ):
        """
        ...
        """
        file_headers = {} if file_headers is None else file_headers
        if fileobj is None:
            fileobj = open(filename, mode or "rb")
        self.fileobj = fileobj

        if version is not None and int(version) not in (1, 2):
    ...
    def _write_header(self):
        ...
        if "ip_address" not in self.file_headers:
            warnings.warn(
                "Using '127.0.0.1' as IP address of machine that's archiving"
            )
        ...
        if self.version == 1:
            ...
            payload = (
                ...
                "Content-type Result-code Checksum Location "
                "Offset Filename Archive-length"
            )
        else:
            raise IOError(
                "Can't write an ARC file with version '\"%s\"'" % self.version
            )
        
        fname = os.path.basename(self.fileobj.name)
        header = ARCHeader(
            url="filedesc://%s" % fname,
    ...
    def _read_file_header(self):
        ...
        if self.version and int(self.version) != version:
            raise IOError(
                (
                    "Version mismatch. Requested version was '%s' "
                    "but version in file was '%s'"
                ) % (self.version, version)
            )
        ...
        if version == '1':
            ...
            self.file_headers = {
                "ip_address": ip_address, "org": organisation,
